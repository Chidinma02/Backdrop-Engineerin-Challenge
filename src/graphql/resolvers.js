...
export const resolvers = {
  Query: {
    async getUser(_parent, { id }, contextValue) {
    ...
    async getUsers(_parent, _args, contextValue) {
      const user = contextValue?.user;
    ...
    async getAccount(_parent, { user_id }, contextValue) {
      const user = contextValue?.user;
      ...
      const bankuser = await BankAccountModel.findOne({
        user: user_id,
      }).populate("user");
      if (!bankuser) {
        throw new Error("Invalid user id");
  ...
  Mutation: {
    ...
    async deleteUser(_, { id }, contextValue) {
      const user = contextValue?.user;
    ...
    async addAccount(_, args, contextValue) {
      const user = contextValue?.user;
      ...
      try {
        ...
        if (!isLevenshteinDistanceWithinAllowableLimit) {
          throw new Error(
            `Account name provided [${args?.user_account_name}] doesn't match the account name linked to this account number [${response?.data?.data?.account_name}]`
          );
        }
